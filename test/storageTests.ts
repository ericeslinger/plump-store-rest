...
export function testSuite(context, storeOpts ) {
  ...
  context.describe(store.name, () => {
    ...
    context.describe('core CRUD', () => {
      ...
      /*
       in the following test, we delete out relationships
       because relationships _can_ be returned in a request for 'attributes'
       but they are not guaranteed to be present. (some models auto-fetch relationships because
       business logic knows that those rels need to be there and they're cheap to get)
       TODO: really need a special test suite for terminal vs cache stores
      */
        ...
        .then((createdObject: ModelData) => {
          ...
          .then((updatedObject: ModelData) => {
            ...
            .then(v => {
              ...
              delete thing.relationships;
              return expect(v).to.deep.include(thing);
            });
          });
        });
      });
